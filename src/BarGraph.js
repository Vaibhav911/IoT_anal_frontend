...
export default function BarGraph(props) {
  // **This component renders grouped bar graphs for quantitative only.**
  ...
  //data type with qualitative one. This might be useful in future
   ...
   seriesData =[]; // this variable stores all the bar graph data which is used by this component for plotting purposes
  ...
  else//else the sensor data type will be qualitative
  {
      for(var k=0;k< props.barGraphData.length;k++)
      {
          seriesData.push({
   ...
   const [state, setState] = React.useState({
    options: {
        ...
        stroke: {
         ...
         xaxis: {// this is where the x axis labels(time stamps) of the bar graph are initialised 
      ...
      series: seriesData,// this initializes the data of the bar graphs for the 0th attribute of given sensor type
   });


 var selectStatType = {}   // stores the statistitic type and attribute type which the user inputs from the dropdown
 //for all the senor sets.
 ...
 var dataType = props.barGraphData[0].data_type//this is the sensor data type(qualt/quant) for 0th sensor set
 //Current system restricts all the sets to have same sensor data type(either all qualitative or all quantitative)
    ...
    for (var i=0;i<props.barGraphData.length;i++)
    {
      ...
      for(var j=0;j<props.barGraphData[i].attributes.length;j++)
      {
          attributeItemArray.push(
            <MenuItem value={props.barGraphData[i].attributes[j]}>{props.barGraphData[i].attributes[j].charAt(0).toUpperCase() + props.barGraphData[i].attributes[j].slice(1)}</MenuItem>
          )
      }
 ...
 {  
  ...
  useEffect(()=>{
    ...
    //user changes either stats type of attribute type
    seriesData=[];
    for(var k=0;k< props.barGraphData.length;k++)
    {
    ...
    setState({
      options: {
        ...
        stroke: {
         ...
         xaxis: {//x coordinates labels of time-frames
